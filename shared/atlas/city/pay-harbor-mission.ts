...
export function projectPayHarborPhysicalMission(state: LastLanternState, builderStationIndex = 0): PayHarborPhysicalMission {
  ...
  if (state.phase === 'selected') return mission(state, restoration, 'REVIEW THE PAYMENT REQUEST', 'NIM is the payment. Lunas are the smaller units. Check the network, recipient, exact amount, and confirmation rule before approving.', 'EXPLORER PATH / VERIFY BEFORE APPROVAL', 'payment-review', 'Review');
  if (state.phase === 'review') return mission(state, restoration, 'CONFIRM THE SAFE PAYMENT', state.mode === 'practice' ? 'Practice mode uses safe local evidence. It creates no payment, score, or reward claim.' : 'Approve only the exact TestAlbatross request, then wait for payment confirmation.', 'PAYMENT REVIEW / CHECK BEFORE APPROVAL', 'payment-review', state.mode === 'practice' ? 'Confirm practice' : 'Pay with Nimiq Pay');
  if (state.phase === 'confirming') return mission(state, restoration, 'WAIT FOR PAYMENT CONFIRMATION', 'The wallet receipt is not proof of delivery. Atlas is checking the current network record.', 'CHECKING / ROUTE STILL CLOSED', 'payment-review', state.mode === 'practice' ? 'Confirm practice' : 'Check confirmation');
